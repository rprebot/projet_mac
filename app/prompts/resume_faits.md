## 📋 TÂCHE

À partir de la section "FAITS" d'une conclusion judiciaire, vous devez produire un résumé chronologique exhaustif et factuel.

---

## ⚠️ RÈGLE ABSOLUE DE FORMAT

**Votre réponse doit contenir UNIQUEMENT le résumé des faits.**

Vous ne devez JAMAIS inclure dans votre réponse :
- Des étapes de raisonnement ("Étape 1", "J'ai lu...", etc.)
- Des explications sur votre méthode ("J'ai identifié...", "La section contient...")
- Des commentaires sur le document source
- Des objectifs ou rappels de consignes

Produisez directement le résumé formaté, rien d'autre.

---

## ⚙️ INSTRUCTIONS GÉNÉRALES

* Rester objectif et factuel
* Conserver les références légales exactes
* Maintenir la terminologie juridique appropriée
* Ne pas ajouter d'interprétation personnelle
* Conserver les montants, dates et noms exacts
* **ÊTRE CONCIS : le résumé des faits fera 10 à 15 lignes MAXIMUM**
* Se concentrer uniquement sur les faits essentiels au litige

---

## 📄 FORMAT DU RÉSUMÉ ATTENDU

### I - RÉSUMÉ DES FAITS

Narration chronologique, exhaustive et factuelle de TOUS les événements pertinents qui ont conduit au litige.

**Principes de structuration :**

* Respecter l'ordre chronologique et indiquer les dates exactes (jours, mois, années)
* Pour les événements importants, reprendre les dates de façon la plus précise possible
* Si le document source utilise une structure particulière, la respecter
* Si aucune structure n'est imposée, organiser de manière logique et fluide

**Format de rédaction :**

* Rédiger en paragraphes fluides et narratifs
* Utiliser le passé composé ou le passé simple
* Éviter les listes à puces (sauf si nécessaire pour la clarté)
* Privilégier la narration continue
* **10 à 15 LIGNES MAXIMUM - Être synthétique**